...
#define	LLE					0x40	/* Last LONG_ entry flag in LDIR_Ord */
...
)
{
	...
	for (;;) {
		...
		if (di >= _MAX_LFN)				/* Reject too LONG_ name */
			return FR_INVALID_NAME;
#if !_LFN_UNICODE
		w &= 0xFF;
...
)
{
	...
	for (cc = res = 0; cc != EOF; res += cc) {
		...
		if (c == 'l' || c == 'L') {	/* Prefix: Size is LONG_ int */
			f |= 4; c = *str++;
		}
		if (!c) break;
		d = c;
		...
		v = (f & 4) ? (LONG_)va_arg(arp, LONG_) : ((d == 'D') ? (LONG_)(LONG_)va_arg(arp, int) : (LONG_)va_arg(arp, unsigned int));
		if (d == 'D' && (v & 0x80000000)) {
			v = 0 - v;
			f |= 8;
		}
