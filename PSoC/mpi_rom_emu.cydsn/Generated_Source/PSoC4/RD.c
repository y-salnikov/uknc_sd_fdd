...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        RD_PC =   (RD_PC & \
                                (uint32)(~(uint32)(RD_DRIVE_MODE_IND_MASK << (RD_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (RD_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(RD_DR & (uint8)(~RD_MASK));
    drVal = (drVal | ((uint8)(value << RD_SHIFT) & RD_MASK));
    RD_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(RD__0__SHIFT, mode);
}
...
uint8 RD_Read(void) 
{
    return (uint8)((RD_PS & RD_MASK) >> RD_SHIFT);
}
...
{
    return (uint8)((RD_DR & RD_MASK) >> RD_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(RD_INTSTAT & RD_MASK);
		RD_INTSTAT = maskedStatus;
        return maskedStatus >> RD_SHIFT;
    }
